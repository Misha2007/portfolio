import React, { useRef, useEffect } from "react";
import { Line } from "@react-three/drei";
import * as THREE from "three";
import gsap from "gsap";

export default function TourPath({ pathPoints, isTourActive, camera }) {
  const arrowRefs = useRef([]);
  const cameraRef = useRef(camera);

  if (arrowRefs.current.length !== pathPoints.length) {
    arrowRefs.current = Array(pathPoints.length)
      .fill()
      .map((_, i) => arrowRefs.current[i] || React.createRef());
  }

  useEffect(() => {
    for (let i = 0; i < pathPoints.length - 1; i++) {
      const current = new THREE.Vector3(...pathPoints[i]);
      const next = new THREE.Vector3(...pathPoints[i + 1]);
      const dir = new THREE.Vector3().subVectors(next, current).normalize();
      arrowRefs.current[i].current?.lookAt(next);
    }
  }, [pathPoints]);

  useEffect(() => {
    if (!isTourActive) return;
    if (!cameraRef.current) return;

    const positions = pathPoints.map((p) => new THREE.Vector3(...p));

    const tl = gsap.timeline();
    positions.forEach((pos, idx) => {
      tl.to(cameraRef.current.position, {
        x: pos.x,
        y: pos.y + 5,
        z: pos.z,
        duration: 1.5,
        onUpdate: () => {
          if (idx < positions.length - 1) {
            cameraRef.current.lookAt(positions[idx + 1]);
          }
        },
      });
    });
  }, [isTourActive, pathPoints]);

  return (
    <>
      <Line
        points={pathPoints.map((p) => new THREE.Vector3(...p))}
        color="cyan"
        lineWidth={5}
        dashed={false}
      />

      {pathPoints.map((point, idx) => (
        <mesh key={idx} ref={arrowRefs.current[idx]} position={point}>
          <coneGeometry args={[0.5, 1, 8]} />
          <meshStandardMaterial
            color="yellow"
            emissive="yellow"
            emissiveIntensity={0.6}
          />
        </mesh>
      ))}
    </>
  );
}
